...
describe('parseRules', () => {
    ...
    it('parse comment', async () => {
        ...
        const rule = parser.Parse(
            'name = "text1" (* "some" "comment" * *) "text3"; (* name2 = "comment" *)'
        ).rules;
    ...
    it('parse reference', async () => {
        const parser = new RuleParser();
        const rule = parser.Parse('name = "t1" name1 | "t2"; name1 = "t3" ')
            .rules;
        assert.strictEqual(rule.length, 2);
        const rule0 = rule[0];
